...
export default async function HomePage({ params }: { params: Promise<{ locale: Locale }> }) {
  ...
  return (
    <>
      <section className="overflow-hidden bg-[radial-gradient(circle_at_92%_8%,#cdefff_0%,transparent_30%),linear-gradient(135deg,#f6fbff_0%,#ffffff_45%,#e8f6fb_100%)]">
        <div className="mx-auto grid max-w-[1480px] items-center gap-10 px-5 py-8 sm:px-8 lg:grid-cols-[0.88fr_1.12fr] lg:py-10">
          <div>
            <p className="text-lg font-black uppercase tracking-wide text-teal">{hero.eyebrow}</p>
            <h1 className="mt-4 max-w-3xl text-4xl font-black leading-[1.08] text-[#0D1B2A] sm:text-5xl lg:text-[3rem]">{hero.title}</h1>
            <p className="mt-5 max-w-2xl text-lg leading-8 text-slate-700">{hero.subtitle}</p>
            <p className="mt-2 max-w-2xl text-base leading-7 text-slate-700">{hero.note}</p>
            <div className="mt-6 grid gap-3 sm:grid-cols-3">
              {t.home.badges.map((badge) => (
                <div key={badge} className="flex items-center gap-3">
                  <span className="flex h-10 w-10 items-center justify-center rounded-full bg-white text-sm font-black text-teal shadow-lg shadow-slate-900/10 ring-1 ring-slate-200">✓</span>
                  <span className="text-sm font-black leading-5 text-ocean">{badge}</span>
                </div>
              ))}
            </div>
            <div className="mt-7 flex flex-col gap-4 sm:flex-row">
              <Link href={`/${locale}/book`} className="rounded-md bg-teal px-7 py-3.5 text-center font-black text-white shadow-xl shadow-teal/20 hover:bg-ocean">
